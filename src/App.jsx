...
export default function App() {
  ...
  // Use lazy initializers so demo data is ready on the very first render —
  // avoids a null intermediate state that triggered an "undefined" read in some browsers.
  const [mixtureData, setMixtureData] = useState(() => {
    const synth = generateSyntheticData();
    return {
      wavelengths: synth.wavelengths,
      times: synth.times,
      mixtures: synth.mixtures,
      _trueConcentrations: synth.concentrations,
      _truePureSpectra: synth.pureSpectra,
    };
  });
  ...
  const concentrationChartData = useMemo(() => {
    if (!results || !mixtureData || !results.C || !mixtureData.times) return [];
    return mixtureData.times.map((t, ti) => {
      const row = { time: t };
      results.speciesNames.forEach((name, i) => {
        row[name] = results.C[i]?.[ti] ?? 0;
      });
      // include true values if demo
      if (mode === 'demo' && referenceData?.trueConcentrations?.[ti]) {
        results.speciesNames.forEach((name, i) => {
          row[name + '_true'] = referenceData.trueConcentrations[ti]?.[i] ?? 0;
        });
      }
      return row;
    });
  ...
  const spectraChartData = useMemo(() => {
    if (!results || !mixtureData || !results.S || !mixtureData.wavelengths) return [];
    return mixtureData.wavelengths.map((w, wi) => {
      const row = { wavelength: w };
      results.speciesNames.forEach((name, i) => {
        row[name] = results.S[wi]?.[i] ?? 0;
      });
      return row;
    });
  }, [results, mixtureData]);
  ...
  const mixtureChartData = useMemo(() => {
    if (!mixtureData || !mixtureData.wavelengths || !mixtureData.times || !mixtureData.mixtures) return [];
    return mixtureData.wavelengths.map((w, wi) => {
      const row = { wavelength: w };
      mixtureData.times.forEach((t, ti) => {
        row['t' + ti] = mixtureData.mixtures[wi]?.[ti] ?? 0;
      });
      return row;
    });
  }, [mixtureData]);
